import { useState } from "react";
import Navbar from "../components/Navbar";
import Footer from "../components/Footer";
import ProductCard from "../components/ProductCard";
import Button from "../components/ui/Button";
import PageHero from "../components/PageHero";
import { ArrowLineUpRightIcon, FunnelIcon, XIcon } from "@phosphor-icons/react";
import product1 from "../assets/prod-1.webp";
import heroBg from "../assets/hero-bg.webp";
import heroBg2 from "../assets/hero-bg2.webp";

const CATEGORIES = ["All", "Women's", "Men's", "Sports", "Body Shapers", "Accessories"];
const SORT_OPTIONS = ["Newest First", "Price: Low to High", "Price: High to Low", "Best Selling"];
const SIZES = ["XS", "S", "M", "L", "XL", "XXL"];
const COLORS = ["#000000", "#FFFFFF", "#ef4444", "#00864A", "#533113", "#3b82f6"];

const PRODUCTS = Array.from({ length: 12 }, (_, i) => ({
  id: i + 1,
  image: product1,
  name: "Women's Sports Wear",
  price: "gh₵ 120.00",
  category: i % 3 === 0 ? "Women's" : i % 3 === 1 ? "Men's" : "Sports",
}));

function NewArrivals() {
  const [activeFilter, setActiveFilter] = useState("All");
  const [activeSort, setActiveSort] = useState("Newest First");
  const [filterPanelOpen, setFilterPanelOpen] = useState(false);
  const [selectedSizes, setSelectedSizes] = useState<string[]>([]);
  const [selectedColors, setSelectedColors] = useState<string[]>([]);
  const [priceMin, setPriceMin] = useState(0);
  const [priceMax, setPriceMax] = useState(500);

  const filtered =
    activeFilter === "All" ? PRODUCTS : PRODUCTS.filter((p) => p.category === activeFilter);

  const toggleSize = (s: string) =>
    setSelectedSizes((prev) => (prev.includes(s) ? prev.filter((x) => x !== s) : [...prev, s]));

  const toggleColor = (c: string) =>
    setSelectedColors((prev) => (prev.includes(c) ? prev.filter((x) => x !== c) : [...prev, c]));

  return (
    <div className="min-h-screen bg-[#FFFBF6]">
      <Navbar />

      {/* ── Hero ── */}
      <PageHero
        bgImage={heroBg}
        bgPosition="50% 35%"
        title={"NEW\nARRIVALS"}
        subtitle="The latest sports, women's and many more accessories — fresh drops, just in."
        badge="FitwearGH Collection"
        ctaText="Shop the Drop"
        image1={product1}
        image2={heroBg2}
      />

      {/* ── Filter / Sort Bar ── */}
      <div className="max-w-[1440px] 2xl:max-w-[1620px] mx-auto px-4 md:px-10 mt-8">

        {/* ── MOBILE: sort + filter button only ── */}
        <div className="flex md:hidden items-center gap-3 pb-4 border-b border-[#DEDEDE]">
          <select
            value={activeSort}
            onChange={(e) => setActiveSort(e.target.value)}
            className="raleway-light text-sm border border-[#DEDEDE] text-[#533113] bg-transparent px-3 py-2 outline-none cursor-pointer flex-1"
          >
            {SORT_OPTIONS.map((o) => (
              <option key={o} value={o}>{o}</option>
            ))}
          </select>

          <button
            onClick={() => setFilterPanelOpen(!filterPanelOpen)}
            className="flex items-center gap-2 bg-[#533113] text-white px-4 py-2 raleway-light text-sm shrink-0"
          >
            {filterPanelOpen ? <XIcon size={16} /> : <FunnelIcon size={16} />}
            Filters
          </button>
        </div>

        {/* ── DESKTOP: category pills + sort + filter button ── */}
        <div className="hidden md:flex flex-col gap-4">
          {/* Category pills */}
          <div className="flex items-center gap-3 overflow-x-auto pb-1">
            {CATEGORIES.map((f) => (
              <button
                key={f}
                onClick={() => setActiveFilter(f)}
                className={`whitespace-nowrap raleway-light text-sm px-5 py-2 border transition-all duration-200 ${
                  activeFilter === f
                    ? "bg-[#533113] text-white border-[#533113]"
                    : "bg-transparent text-[#533113] border-[#533113] hover:bg-[#533113]/10"
                }`}
              >
                {f}
              </button>
            ))}
          </div>

          {/* Sort + filter row */}
          <div className="flex justify-between items-center border-t border-[#DEDEDE] pt-4">
            <p className="raleway-light text-sm text-[#533113]/60">{filtered.length} items</p>
            <div className="flex items-center gap-3">
              <select
                value={activeSort}
                onChange={(e) => setActiveSort(e.target.value)}
                className="raleway-light text-sm border border-[#533113] text-[#533113] bg-transparent px-3 py-2 outline-none cursor-pointer"
              >
                {SORT_OPTIONS.map((o) => (
                  <option key={o} value={o}>{o}</option>
                ))}
              </select>

              <button
                onClick={() => setFilterPanelOpen(!filterPanelOpen)}
                className="flex items-center gap-2 bg-[#533113] text-white px-4 py-2 raleway-light text-sm"
              >
                {filterPanelOpen ? <XIcon size={16} /> : <FunnelIcon size={16} />}
                {filterPanelOpen ? "Close" : "Filters"}
              </button>
            </div>
          </div>
        </div>

        {/* ── Expandable filter panel (both mobile + desktop) ── */}
        {filterPanelOpen && (
          <div className="border border-[#DEDEDE] bg-white p-6 mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">

            {/* Category — shown in panel on mobile only */}
            <div className="flex flex-col gap-3 md:hidden">
              <p className="raleway-bold text-sm text-[#533113] uppercase tracking-widest">Category</p>
              <div className="flex flex-wrap gap-2">
                {CATEGORIES.map((f) => (
                  <button
                    key={f}
                    onClick={() => setActiveFilter(f)}
                    className={`raleway-light text-xs px-3 py-1 border transition-all duration-150 ${
                      activeFilter === f
                        ? "bg-[#533113] text-white border-[#533113]"
                        : "text-[#533113] border-[#533113] hover:bg-[#533113]/10"
                    }`}
                  >
                    {f}
                  </button>
                ))}
              </div>
            </div>

            {/* Size */}
            <div className="flex flex-col gap-3">
              <p className="raleway-bold text-sm text-[#533113] uppercase tracking-widest">Size</p>
              <div className="flex flex-wrap gap-2">
                {SIZES.map((s) => (
                  <button
                    key={s}
                    onClick={() => toggleSize(s)}
                    className={`border px-3 py-1 raleway-light text-xs transition-all duration-150 ${
                      selectedSizes.includes(s)
                        ? "bg-[#533113] text-white border-[#533113]"
                        : "text-[#533113] border-[#533113] hover:bg-[#533113]/10"
                    }`}
                  >
                    {s}
                  </button>
                ))}
              </div>
            </div>

            {/* Color */}
            <div className="flex flex-col gap-3">
              <p className="raleway-bold text-sm text-[#533113] uppercase tracking-widest">Color</p>
              <div className="flex flex-wrap gap-3">
                {COLORS.map((c) => (
                  <button
                    key={c}
                    onClick={() => toggleColor(c)}
                    style={{ backgroundColor: c }}
                    className={`w-7 h-7 rounded-full transition-all duration-150 ${
                      selectedColors.includes(c)
                        ? "ring-2 ring-[#533113] ring-offset-2 scale-110"
                        : "border border-[#DEDEDE] hover:scale-110"
                    }`}
                  />
                ))}
              </div>
            </div>

            {/* Price range — dual sliders */}
            <div className="flex flex-col gap-4 sm:col-span-2 md:col-span-1">
              <p className="raleway-bold text-sm text-[#533113] uppercase tracking-widest">Price Range</p>

              {/* Min slider */}
              <div className="flex flex-col gap-1">
                <div className="flex justify-between">
                  <span className="raleway-light text-xs text-[#533113]/60">Min</span>
                  <span className="raleway-bold text-xs text-[#533113]">gh₵ {priceMin}</span>
                </div>
                <input
                  type="range"
                  min={0}
                  max={500}
                  step={10}
                  value={priceMin}
                  onChange={(e) => setPriceMin(Math.min(Number(e.target.value), priceMax - 10))}
                  className="w-full accent-[#533113] cursor-pointer"
                />
              </div>

              {/* Max slider */}
              <div className="flex flex-col gap-1">
                <div className="flex justify-between">
                  <span className="raleway-light text-xs text-[#533113]/60">Max</span>
                  <span className="raleway-bold text-xs text-[#533113]">gh₵ {priceMax}</span>
                </div>
                <input
                  type="range"
                  min={0}
                  max={500}
                  step={10}
                  value={priceMax}
                  onChange={(e) => setPriceMax(Math.max(Number(e.target.value), priceMin + 10))}
                  className="w-full accent-[#533113] cursor-pointer"
                />
              </div>

              {/* Range label */}
              <p className="raleway-light text-xs text-[#533113]/70">
                gh₵ {priceMin} — gh₵ {priceMax}
              </p>

              <Button text="Apply" width="w-full" icon={<ArrowLineUpRightIcon size={14} />} />
            </div>
          </div>
        )}
      </div>

      {/* ── Product Grid ── */}
      <main className="max-w-[1440px] 2xl:max-w-[1620px] mx-auto px-4 md:px-10 mt-8 mb-16">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 gap-4 md:gap-6">
          {filtered.map((product) => (
            <ProductCard
              key={product.id}
              image={product.image}
              name={product.name}
              price={product.price}
            />
          ))}
        </div>

        <div className="flex justify-center mt-12">
          <Button
            text="Load More"
            width="w-48 md:w-56"
            icon={<ArrowLineUpRightIcon size={20} />}
          />
        </div>
      </main>

      <Footer />
    </div>
  );
}

export default NewArrivals;

...
// Shown while Firestore loads or if collection is empty
...
function NewArrivals() {
  ...
  }, [activeFilter, activeSort, priceMin, priceMax, selectedColors, selectedSizes]);
          ...
          <div className="flex items-center gap-3 overflow-x-auto pb-1">
            {CATEGORIES.map((f) => (
              <button
                ...
                className={`whitespace-nowrap raleway-regular text-base px-5 py-2 border transition-all duration-200 ${
                  activeFilter === f
        ...
        {filterPanelOpen && (
          <div className="border border-[#DEDEDE] bg-white p-5 md:p-6 mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
            ...
            <div className="flex flex-col gap-3 md:hidden">
              <p className="raleway-bold text-xs text-[#533113] uppercase tracking-widest">
                Category
              </p>
              <div className="flex flex-wrap gap-2">
                {CATEGORIES.map((f) => (
                  <button
                    key={f}
                    onClick={() => setActiveFilter(f)}
                    className={`raleway-regular text-sm px-3 py-1.5 border transition-all ${
                      activeFilter === f
                        ? "bg-[#533113] text-white border-[#533113]"
            ...
            <div className="flex flex-col gap-4 sm:col-span-2 md:col-span-1">
              ...
              <div className="flex flex-col gap-1">
                <div className="flex justify-between">
                  <span className="raleway-regular text-sm text-[#533113]/60">
                    Min
                  </span>
        ...
        {loading ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 gap-4 md:gap-6">
            {Array.from({ length: 8 }).map((_, i) => (
              <div
                key={i}
                className="aspect-[3/4] bg-[#F5EDE0] animate-pulse"
              />
            ))}
          </div>
        ) : sorted.length === 0 ? (
          <div className="text-center py-20">
            <p className="raleway-regular text-lg text-[#533113]/50">
